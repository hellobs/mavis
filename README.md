...
默认的 `start.py` 是"先跑后放"：模拟过程没有画面，跑完后再压缩回放。`live.py` 将模拟与
可视化合并到同一个进程，模拟每完成一步就通过 SSE 实时推送给浏览器，实现"边跑边看"：

```
python live.py --name sim-test --start "20250213-09:30" --stride 10 --step 0
```

然后浏览器打开 http://127.0.0.1:5001/ 即可实时观看。参数与 `start.py` 基本一致，
`--step` 设为 `0`（默认）表示持续运行直到手动停止；`--port` 可指定端口（默认 5001，
避免与回放服务 5000 冲突）。模拟过程照常落盘，事后仍可用 `compress.py` + `replay.py` 回放。
